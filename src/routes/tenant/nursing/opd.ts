...
opdRoutes.put('/clinical-info/:id', zValidator('json', updateClinicalInfoSchema), async (c) => {
  ...
  const existing = await c.env.DB.prepare('SELECT 1 FROM cln_patient_clinical_info WHERE id = ? AND tenant_id = ?').bind(id, tenantId).first();
  ...
  if (fields.length > 0) {
    ...
    await c.env.DB.prepare(`UPDATE cln_patient_clinical_info SET ${fields.join(', ')} WHERE id = ? AND tenant_id = ?`).bind(...values).run();
  }
...
opdRoutes.put('/check-in', zValidator('json', checkInSchema), async (c) => {
  ...
  const existing = await c.env.DB.prepare('SELECT 1 FROM visits WHERE id = ? AND tenant_id = ?').bind(visit_id, tenantId).first();
