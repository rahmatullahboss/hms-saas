...
monitoringRoutes.get('/:id', async (c) => {
  ...
  const result = await db.$client.prepare('SELECT * FROM nur_patient_monitoring WHERE id = ? AND tenant_id = ? AND is_active = 1').bind(id, tenantId).first();
...
monitoringRoutes.put('/:id', zValidator('json', updateMonitoringSchema), async (c) => {
  ...
  const id = parseInt(c.req.param('id'));
  if (isNaN(id)) throw new HTTPException(400, { message: 'Invalid ID' });
  const existing = await db.$client.prepare('SELECT 1 FROM nur_patient_monitoring WHERE id = ? AND tenant_id = ? AND is_active = 1').bind(id, tenantId).first();
  ...
  if (fields.length > 0) {
    ...
    await db.$client.prepare(`UPDATE nur_patient_monitoring SET ${fields.join(', ')} WHERE id = ? AND tenant_id = ?`).bind(...values).run();
  }
...
monitoringRoutes.delete('/:id', async (c) => {
  ...
  const existing = await db.$client.prepare('SELECT 1 FROM nur_patient_monitoring WHERE id = ? AND tenant_id = ? AND is_active = 1').bind(id, tenantId).first();
  if (!existing) throw new HTTPException(404, { message: 'Not found' });
  await db.$client.prepare("UPDATE nur_patient_monitoring SET is_active = 0, updated_at = datetime('now') WHERE id = ? AND tenant_id = ?").bind(id, tenantId).run();
  return c.json({ Results: true });
});
