...
marRoutes.get('/:id', async (c) => {
  ...
  const result = await c.env.DB.prepare('SELECT * FROM nur_medication_admin WHERE id = ? AND tenant_id = ? AND is_active = 1').bind(id, tenantId).first();
...
marRoutes.put('/:id', zValidator('json', updateMARSchema), async (c) => {
  ...
  const id = parseInt(c.req.param('id'));
  if (isNaN(id)) throw new HTTPException(400, { message: 'Invalid ID' });
  const existing = await c.env.DB.prepare('SELECT 1 FROM nur_medication_admin WHERE id = ? AND tenant_id = ? AND is_active = 1').bind(id, tenantId).first();
  ...
  if (fields.length > 0) {
    ...
    await c.env.DB.prepare(`UPDATE nur_medication_admin SET ${fields.join(', ')} WHERE id = ? AND tenant_id = ?`).bind(...values).run();
  }
...
marRoutes.delete('/:id', async (c) => {
  ...
  const existing = await c.env.DB.prepare('SELECT 1 FROM nur_medication_admin WHERE id = ? AND tenant_id = ? AND is_active = 1').bind(id, tenantId).first();
  if (!existing) throw new HTTPException(404, { message: 'Not found' });
  await c.env.DB.prepare("UPDATE nur_medication_admin SET is_active = 0, updated_at = datetime('now') WHERE id = ? AND tenant_id = ?").bind(id, tenantId).run();
  return c.json({ Results: true });
});
