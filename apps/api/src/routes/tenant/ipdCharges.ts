...
const ipdCharges = new Hono<{ Bindings: Env; Variables: Variables }>();
...
ipdCharges.post('/', zValidator('json', postChargeSchema), async (c) => {
  const tenantId = c.get('tenantId');
