      ...
      <div className="space-y-5">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <h1 className="text-2xl font-bold text-[var(--color-text)]">Multi-Branch Overview</h1>
            <p className="text-sm text-[var(--color-text-muted)]">Performance across all branches</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            { label: 'Total Revenue', value: fmtTaka(totalRevenue), icon: <DollarSign className="w-5 h-5" />, color: '#088eaf' },
            { label: 'Total Patients', value: totalPatients.toLocaleString(), icon: <Users className="w-5 h-5" />, color: '#6366f1' },
            { label: 'Active Branches', value: branches.filter(b => b.status === 'active').length, icon: <Building2 className="w-5 h-5" />, color: '#10b981' },
            { label: 'Avg Occupancy', value: `${avgOccupancy}%`, icon: <BedDouble className="w-5 h-5" />, color: '#f59e0b' },
              ...
              {branch.stats.beds_total > 0 && (
                <div className="mb-3">
                  <div className="flex justify-between text-xs mb-1">
                    <span className="text-[var(--color-text-muted)]">Bed Occupancy</span>
